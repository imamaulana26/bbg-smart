<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
Class M_group extends CI_Model {
    var $table = 'tbl_group'; // table yang ingin ditampilkan
    var $order = array('group_id' => 'asc', 'group_name' => 'asc');
    var $id = 'id';
    var $column_order = array(null, 'group_id', 'group_name', 'group_title', null);
    var $column_search = array('group_name','group_title');

    function _get_datatable_query(){
        $this->db->from($this->table);
        $this->db->where('IsDelete', 0);
        $i = 0;
        foreach($this->column_search as $item){
            if($_POST['search']['value']){
                if($i === 0){
                    $this->db->group_start();
                    $this->db->like($item, $_POST['search']['value']);
                } else {
                    $this->db->or_like($item, $_POST['search']['value']);
                }

                if(count($this->column_search) -1 == $i) $this->db->group_end();
            }
            $i++;
        }

        if(isset($_POST['order'])){
            $this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        } else if(isset($this->order)){
            $order = $this->order;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }

    function get_datatables(){
        $this->_get_datatable_query();
        if($_POST['length'] != -1){
            $this->db->limit($_POST['length'], $_POST['start']);
            $query = $this->db->get();
            return $query->result_array();
        }
    }

    function count_filtered(){
        $this->_get_datatable_query();
        $query = $this->db->get();
        return $query->num_rows();
    }

    function get_all_data(){
        $this->db->get($this->table);
        return $this->db->count_all_results();
    }
}